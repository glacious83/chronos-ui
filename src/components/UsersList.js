...
export default function UsersList({users, setUsers, errorUsers = [], setErrorUsers}) {
    ...
    useEffect(() => {
        ...
        // fetch departments, companies, titles, and all users (for managers list)
        Promise.all([axiosInstance.get('/api/departments', {headers: {Authorization: `Bearer ${token}`}}), axiosInstance.get('/api/companies', {headers: {Authorization: `Bearer ${token}`}}), axiosInstance.get('/api/rates/titles', {headers: {Authorization: `Bearer ${token}`}}), axiosInstance.get('/api/users', {headers: {Authorization: `Bearer ${token}`}})]).then(([deptRes, compRes, titlesRes, usersRes]) => {
    ...
    const handleApprove = async (userId) => {
        const token = localStorage.getItem('authToken');
        await axiosInstance.put(`/api/admin/users/${userId}/approve?approved=true`, {}, {headers: {Authorization: `Bearer ${token}`}});
        setUsers(u => u.map(x => x.id === userId ? {...x, approved: true} : x));
    };

    const handleActivate = async (userId) => {
        const token = localStorage.getItem('authToken');
        await axiosInstance.put(`/api/admin/users/${userId}/activate?active=true`, {}, {headers: {Authorization: `Bearer ${token}`}});
        setUsers(u => u.map(x => x.id === userId ? {...x, active: true} : x));
    };

    const handleDeactivate = async (userId) => {
        const token = localStorage.getItem('authToken');
        await axiosInstance.put(`/api/admin/users/${userId}/activate?active=false`, {}, {headers: {Authorization: `Bearer ${token}`}});
        setUsers(u => u.map(x => x.id === userId ? {...x, active: false} : x));
    };
    ...
    const handleSave = async () => {
        ...
        const body = {
            ...
            responsibleManagerId: selectedUser.responsibleManagerId ? +selectedUser.responsibleManagerId : null
        };

        await axiosInstance.put(`/api/users/${selectedUser.id}`, body, {headers: {Authorization: `Bearer ${token}`}});
        ...
        setUsers(u => u.map(x => x.id === selectedUser.id ? {
            ...
            responsibleManager: body.responsibleManagerId ? managers.find(m => m.id === body.responsibleManagerId) : null,
            company: companies.find(c => c.id === selectedUser.companyId)
        } : x));

        try {
            const errRes = await axiosInstance.get('/api/admin/checkErrors', {headers: {Authorization: `Bearer ${token}`}});
            setErrorUsers(errRes.data.userError || []);
        } catch (err) {
            console.error('Failed to refresh error list', err);
        }
    ...
    return (<>
        <TableContainer component={Paper}>
            <Table size="small">
                <TableHead>
                    <TableRow>
                        {['First Name', 'Middle', 'Last', 'Email', 'Sydipel', 'SAP ID', 'VM', 'IP', 'Phone', 'Location', 'Title', 'Dept', 'Manager', 'Company', 'Active', 'Approved', 'Actions'].map(label => (
                            <TableCell key={label}>{label}</TableCell>))}
                    </TableRow>
                </TableHead>
                <TableBody>
                    {users.map(user => {
                        // Only mark error if user is active
                        const hasError = user.active && errorUsers.some(e => e.id === user.id);

                        return (<TableRow
                            key={user.id}
                            className={`
     ${hasError ? 'error-row' : ''}
     ${!user.approved ? ' pending-approval' : ''}
   `}
                        >
                            ...
                            <TableCell>
                                {user.responsibleManager ? `${user.responsibleManager.firstName} ${user.responsibleManager.lastName}` : '— none —'}
                            </TableCell>
                            ...
                            <TableCell>
                                ...
                                >
                                    {!user.approved && <MenuItem onClick={() => handleApprove(user.id)}>
                                        <DoneAllIcon fontSize="small"/> Approve
                                    </MenuItem>}
                                    {!user.active && <MenuItem onClick={() => handleActivate(user.id)}>
                                        <LockOpenIcon fontSize="small"/> Activate
                                    </MenuItem>}
                                    {user.active && (<MenuItem onClick={() => handleDeactivate(user.id)}>
                                        <CloseIcon fontSize="small"/> Deactivate
                                    </MenuItem>)}
        ...
        {openEdit && (<Dialog open onClose={() => setOpenEdit(false)}>
            ...
            <DialogContent>
                ...
                >
                    ...
                    {managers.map(m => (<MenuItem key={m.id} value={m.id}>
