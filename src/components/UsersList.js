...
export default function UsersList({ users, setUsers, errorUsers = [] }) {
    ...
    const handleDeactivate = async userId => {
        const token = localStorage.getItem('authToken');
        await axiosInstance.put(
            `/api/admin/users/${userId}/activate?active=false`,
            {},
            { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    const handleSave = async () => {
        ...
        await axiosInstance.put(`/api/users/${selectedUser.id}`, body, {
            headers: { Authorization: `Bearer ${token}` }
        });
            ...
            <TableContainer component={Paper}>
                <Table size="small">
                    <TableHead>
                        <TableRow>
                            {[
                                'First Name','Middle','Last','Email','Sydipel','SAP ID','VM','IP','Phone','Location','Title','Dept','Manager','Company','Active','Approved','Actions'
                            ].map(label => (
                    ...
                    <TableBody>
                        {[...users].sort((a, b) => a.lastName.localeCompare(b.lastName)).map(user => {
                            const hasError = errorUsers.some(
                                e => (typeof e === 'object' ? e.id : e) === user.id
                            );
                            return (
                                <TableRow
                                    key={user.id}
                                    className={
                                        `${hasError ? 'error-row' : ''}${!user.approved ? ' pending-approval' : ''}`
                                    }
                                >
                                    ...
                                    <TableCell>
                                        ...
                                        >
                                            ...
                                            {user.active && (
                                                <MenuItem onClick={() => handleDeactivate(user.id)}>
                ...
                <Dialog open onClose={() => setOpenEdit(false)}>
                    ...
                    <DialogContent>
                        ...
                        <TextField
                            ...
                            label="SAP ID"
                            autoComplete="off"
                            value={selectedUser.sapId}
                            onChange={e => setSelectedUser(s => ({ ...s, sapId: e.target.value }))}
                        />
                        <TextField
                            margin="normal"
                            fullWidth
