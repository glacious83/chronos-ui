            ...
            <List>
                {Array.isArray(menuItems) && menuItems.map((item) => (
                    <React.Fragment key={item.text}>
                        <ListItemButton onClick={() => item.subItems.length > 0 ? handleClick(item.text) : setSelectedMenu(item.text)}>
                            <ListItemIcon>
                                <Badge
                                    color="error"
                                    variant={
                            ...
                            {item.subItems.length > 0 && (
                                <List component="div" disablePadding>
                                    {item.subItems.map((subItem) => (
                                        <ListItemButton key={subItem} sx={{ pl: 4 }} onClick={() => handleSubItemClick(subItem)}>
